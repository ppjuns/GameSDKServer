package com.ppjun.game.base


class Constant {
    companion object {
        const val SUCCESS_CODE = 200
        const val ERROR_CODE = -200
        const val SUCCESS_INIT = "初始化成功"
        const val ERROR_INIT = "初始化失败"
        const val SUCCESS_ADD = "新增成功"
        const val ERROR_ADD = "新增失败"
        const val ERROR_REPECT_ADD = "新增失败，游戏已存在"
        const val ERROR_OPENID_LOGIN = "登录失败,openId 为空"
        const val ERROR_NAME_LOGIN = "登录失败,name 为空"
        const val ERROR_IMG_LOGIN = "登录失败,img 为空"
        const val ERROR_SEX_LOGIN = "登录失败,sex 为空"
        const val SUCCESS_LOGIN = "登录成功"
        const val WECHAT_APP_ID = ""
        const val WECHAT_MCH_ID = ""
        const val WECHAT_MCH_KEY = ""
        const val ALIPAY_APP_ID = "2018072060691431"
        const val ALIPAY_PUBLIC_KEY =""
        const val TRADE_TYPE = "trade_type"
        const val PAGE_SIZE = 10
        const val PUBLIC_KEY = ""
        const val PRIVATE_KEY = ""
    }
}